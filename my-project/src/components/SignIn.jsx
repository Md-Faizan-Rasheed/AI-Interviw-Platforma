...
const SignIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Youe are in sign page333");

      const result = await response.json();
      const { sucess, message, jwtToken, name, error } = result;
      console.log("Youe are in sign page63636",result);

      if (sucess) {
        handleSucess(message);
        localStorage.setItem("token", jwtToken);
        localStorage.setItem("loggedInUser", name);
        login();  // Call login function instead of setIsLoggedIn(true)

        // setIsLoggedIn(true); // Update global state
        // setTimeout(() => {
          navigate("/dashboard");
          console.log("yOuear ein sig a23");
